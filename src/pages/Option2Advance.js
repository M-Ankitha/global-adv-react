import styles from "./Option2Advance.module.css";

const Option2Advance = () => {
  return (
    <div className={styles.option2Advance1}>
      <div className={styles.defaultToggle}>
        <div className={styles.widgets}>
          <div className={styles.widget6}>
            <div className={styles.col5}>
              <div className={styles.row5}>
                <img
                  className={styles.buttonTrashIcon10}
                  alt=""
                  src="/button-trash@2x.png"
                />
                <img
                  className={styles.buttonEditIcon10}
                  alt=""
                  src="/button-edit@2x.png"
                />
                <img
                  className={styles.buttonSettingsIcon10}
                  alt=""
                  src="/button-settings@2x.png"
                />
              </div>
              <div className={styles.row4}>
                <img
                  className={styles.buttonTrashIcon10}
                  alt=""
                  src="/button-trash@2x.png"
                />
                <img
                  className={styles.buttonEditIcon10}
                  alt=""
                  src="/button-edit@2x.png"
                />
                <img
                  className={styles.buttonSettingsIcon10}
                  alt=""
                  src="/button-settings@2x.png"
                />
              </div>
              <div className={styles.row3}>
                <img
                  className={styles.buttonTrashIcon10}
                  alt=""
                  src="/button-trash@2x.png"
                />
                <img
                  className={styles.buttonEditIcon10}
                  alt=""
                  src="/button-edit@2x.png"
                />
                <img
                  className={styles.buttonSettingsIcon10}
                  alt=""
                  src="/button-settings@2x.png"
                />
              </div>
              <div className={styles.row2}>
                <img
                  className={styles.buttonTrashIcon10}
                  alt=""
                  src="/button-trash@2x.png"
                />
                <img
                  className={styles.buttonEditIcon10}
                  alt=""
                  src="/button-edit@2x.png"
                />
                <img
                  className={styles.buttonSettingsIcon10}
                  alt=""
                  src="/button-settings@2x.png"
                />
              </div>
              <div className={styles.row1}>
                <img
                  className={styles.buttonTrashIcon10}
                  alt=""
                  src="/button-trash@2x.png"
                />
                <img
                  className={styles.buttonEditIcon10}
                  alt=""
                  src="/button-edit@2x.png"
                />
                <img
                  className={styles.buttonSettingsIcon10}
                  alt=""
                  src="/button-settings@2x.png"
                />
              </div>
            </div>
            <div className={styles.col4}>
              <div className={styles.button2}>
                <div className={styles.background32} />
                <div className={styles.inProgress}>In Progress</div>
              </div>
              <div className={styles.button3}>
                <div className={styles.background33} />
                <div className={styles.rejected}>Rejected</div>
              </div>
              <div className={styles.button4}>
                <div className={styles.background34} />
                <div className={styles.success}>Success</div>
              </div>
              <div className={styles.button5}>
                <div className={styles.background35} />
                <div className={styles.inProgress}>In Progress</div>
              </div>
              <div className={styles.button6}>
                <div className={styles.background36} />
                <div className={styles.approved}>Approved</div>
              </div>
            </div>
            <div className={styles.col3}>
              <div className={styles.reactjsRuby}>{`ReactJS, Ruby `}</div>
              <div className={styles.angularjsC}>AngularJS, C#</div>
              <div className={styles.laravelMetronic}>Laravel, Metronic</div>
              <div className={styles.pythonMysql}>Python, MySQL</div>
              <div className={styles.reactjsHtml}>ReactJs, HTML</div>
            </div>
            <div className={styles.col2}>
              <div className={styles.item5}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$45,200,000</div>
              </div>
              <div className={styles.item4}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$57,000</div>
              </div>
              <div className={styles.item3}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$560,000</div>
              </div>
              <div className={styles.item2}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$4,600,000</div>
              </div>
              <div className={styles.item1}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$2,000,000</div>
              </div>
            </div>
            <div className={styles.col1}>
              <div className={styles.item51}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.ktrMobileApplication}>
                  KTR Mobile Application
                </div>
                <div className={styles.item5Child} />
                <img
                  className={styles.kickstarterIcon2}
                  alt=""
                  src="/187kickstarter@2x.png"
                />
              </div>
              <div className={styles.item41}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.hrManagement}>HR Management</div>
                <div className={styles.item5Child} />
                <img
                  className={styles.kickstarterIcon2}
                  alt=""
                  src="/182bebo@2x.png"
                />
              </div>
              <div className={styles.item31}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.ciscoManagement}>Cisco Management</div>
                <div className={styles.item5Child} />
                <img
                  className={styles.kickstarterIcon2}
                  alt=""
                  src="/194puzzle@2x.png"
                />
              </div>
              <div className={styles.item21}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.hrManagement}>Telegram Application</div>
                <div className={styles.item5Child} />
                <img
                  className={styles.telegramIcon2}
                  alt=""
                  src="/186telegram@2x.png"
                />
              </div>
              <div className={styles.item11}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.santOutstanding}>Sant Outstanding</div>
                <div className={styles.item5Child} />
                <img
                  className={styles.telegramIcon2}
                  alt=""
                  src="/184plurk@2x.png"
                />
              </div>
            </div>
            <div className={styles.head}>
              <div className={styles.tabs}>
                <div className={styles.background37} />
                <div className={styles.day}>Day</div>
                <div className={styles.week}>Week</div>
                <div className={styles.month}>Month</div>
              </div>
              <div className={styles.newArrivals}>New Arrivals</div>
              <div className={styles.moreThan4004}>
                More than 400+ new members
              </div>
            </div>
          </div>
          <div className={styles.widget5}>
            <div className={styles.item52}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" src="/icon@2x.png" />
            </div>
            <div className={styles.item42}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" src="/icon@2x.png" />
            </div>
            <div className={styles.item32}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" src="/icon@2x.png" />
            </div>
            <div className={styles.item22}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" src="/icon@2x.png" />
            </div>
            <div className={styles.item12}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" src="/icon@2x.png" />
            </div>
            <div className={styles.head1}>
              <div className={styles.dots}>
                <div className={styles.dotsChild} />
                <div className={styles.dotsItem} />
              </div>
              <div className={styles.salesProgress}>Sales Progress</div>
            </div>
          </div>
          <div className={styles.widget2}>
            <div className={styles.item43}>
              <div className={styles.outlinesKeepYou5}>
                Outlines keep you honest. Indulging in poorly driving and keep
                structure keep you honest great
              </div>
              <div className={styles.item4Item} />
              <div className={styles.item4Inner} />
              <div className={styles.div5}>16:50</div>
            </div>
            <div className={styles.item33}>
              <div className={styles.makeDepositUsdContainer}>
                <span>{`Make deposit `}</span>
                <span className={styles.usd700}>USD 700</span>
                <span> to ESL</span>
              </div>
              <div className={styles.item3Item} />
              <div className={styles.item3Inner} />
              <div className={styles.div6}>14:37</div>
            </div>
            <div className={styles.item23}>
              <img className={styles.userIcon4} alt="" />
              <div className={styles.makeDepositUsdContainer}>
                AEOL meeting with
              </div>
              <div className={styles.item2Item} />
              <div className={styles.item2Inner} />
              <div className={styles.div6}>10:00</div>
            </div>
            <div className={styles.item13}>
              <div className={styles.outlinesKeepYou5}>
                Outlines keep you honest. Indulging in poorly driving and keep
                structure
              </div>
              <div className={styles.item1Item} />
              <div className={styles.item1Inner} />
              <div className={styles.div5}>08:42</div>
            </div>
            <div className={styles.head2}>
              <div className={styles.sales}>890,344 Sales</div>
              <div className={styles.recentActivities}>Recent Activities</div>
              <div className={styles.dots1}>
                <div className={styles.dotsChild} />
                <div className={styles.dotsItem} />
              </div>
            </div>
          </div>
          <div className={styles.widget1}>
            <div className={styles.background38} />
            <div className={styles.top}>
              <img className={styles.topChild} alt="" src="/vector-5@2x.png" />
              <img className={styles.topItem} alt="" src="/vector-6@2x.png" />
              <div className={styles.exportButton}>
                <div className={styles.button7} />
                <div className={styles.export}>Export</div>
                <img className={styles.arrowIcon2} alt="" src="/arrow@2x.png" />
              </div>
              <div className={styles.categories}>Categories</div>
            </div>
          </div>
        </div>
        <div className={styles.sidebar}>
          <div className={styles.backgroundRectangle2} />
          <div className={styles.highlightedRectangle3} />
          <img className={styles.topPartIcon} alt="" src="/top-part@2x.png" />
          <div className={styles.iconsBright}>
            <img className={styles.layersIcon2} alt="" src="/layers@2x.png" />
            <img
              className={styles.layout4Blocks2}
              alt=""
              src="/layout-4-blocks@2x.png"
            />
          </div>
          <div className={styles.headings}>
            <div className={styles.custom}>CUSTOM</div>
          </div>
          <div className={styles.subheadings}>
            <div className={styles.users}>Users</div>
            <div className={styles.contacts}>Contacts</div>
            <div className={styles.chat}>Chat</div>
            <div className={styles.pages}>Pages</div>
          </div>
          <div className={styles.subheadingsActive}>
            <div className={styles.dashboard}>Dashboard</div>
            <div className={styles.dashboard1}>DASHBOARD</div>
            <div className={styles.applications}>Applications</div>
          </div>
          <div className={styles.strokes}>
            <div className={styles.strokesChild} />
            <div className={styles.strokesItem} />
            <div className={styles.strokesInner} />
          </div>
          <div className={styles.iconsFaded}>
            <div className={styles.box3}>
              <div className={styles.bound6} />
            </div>
            <img
              className={styles.barcodeReadIcon1}
              alt=""
              src="/barcode-read@2x.png"
            />
          </div>
          <img className={styles.arrowsIcon} alt="" src="/arrows@2x.png" />
        </div>
        <div className={styles.sunheader}>
          <div className={styles.background39} />
          <div className={styles.left}>
            <div className={styles.dashboard2}>Dashboard</div>
            <div className={styles.addNewButton2}>
              <div className={styles.addNewButtonChild} />
              <div className={styles.addNew}>Add New</div>
            </div>
            <div className={styles.leftChild} />
          </div>
          <div className={styles.right}>
            <div className={styles.today}>
              <div className={styles.today1}>Today</div>
              <div className={styles.todayActive}>
                <div className={styles.todayActiveChild} />
                <div className={styles.today1}>Today</div>
              </div>
            </div>
            <div className={styles.month1}>
              <div className={styles.monthActive}>
                <div className={styles.monthActiveChild} />
                <div className={styles.month2}>{`Month `}</div>
              </div>
              <div className={styles.month3}>{`Month `}</div>
            </div>
            <div className={styles.year}>
              <div className={styles.year1}>Year</div>
              <div className={styles.yearActive}>
                <div className={styles.monthActiveChild} />
                <div className={styles.yearActive1}>Year</div>
              </div>
            </div>
            <div className={styles.actionsButtonPassive2}>
              <div className={styles.todayActiveChild} />
              <div className={styles.actions}>Actions</div>
            </div>
            <div className={styles.actionsButtonActive2}>
              <div className={styles.actionsButtonActiveChild} />
              <div className={styles.actions1}>Actions</div>
              <img className={styles.sort1Icon2} alt="" />
            </div>
            <img
              className={styles.filePlusIcon2}
              alt=""
              src="/file-plus-icon@2x.png"
            />
          </div>
        </div>
        <div className={styles.header}>
          <div className={styles.sep2} />
          <div className={styles.right1}>
            <div className={styles.icons}>
              <img className={styles.searchIcon2} alt="" src="/search@2x.png" />
              <img
                className={styles.compilingIcon3}
                alt=""
                src="/compiling@2x.png"
              />
              <div className={styles.equalizer}>
                <div className={styles.bound6} />
                <div className={styles.rectangle62Copy8} />
                <div className={styles.rectangle62Copy25} />
                <div className={styles.rectangle62Copy45} />
                <div className={styles.rectangle62Copy35} />
              </div>
              <img
                className={styles.stockholmIconsShoppingC2}
                alt=""
                src="/stockholm-icons--shopping--cart3@2x.png"
              />
              <img
                className={styles.layout4BlocksIcon3}
                alt=""
                src="/layout4blocks@2x.png"
              />
            </div>
            <div className={styles.nickThomas}>Nick Thomas</div>
            <div className={styles.initialIcon}>
              <div className={styles.rectangle56} />
              <div className={styles.n}>N</div>
            </div>
          </div>
        </div>
      </div>
      <div className={styles.carts}>
        <div className={styles.cart4}>
          <div className={styles.background40} />
          <div className={styles.bugReports}>Bug Reports</div>
          <img
            className={styles.stockholmIconsCommunicatio2}
            alt=""
            src="/stockholm-icons--communication--urgent-mail@2x.png"
          />
        </div>
        <div className={styles.cart3}>
          <div className={styles.background41} />
          <div className={styles.itemOrders}>Item Orders</div>
          <img
            className={styles.compilingIcon4}
            alt=""
            src="/compiling@2x.png"
          />
        </div>
        <div className={styles.cart2}>
          <div className={styles.background42} />
          <div className={styles.newUsers}>New Users</div>
          <img className={styles.addUserIcon1} alt="" src="/adduser@2x.png" />
        </div>
        <div className={styles.cart1}>
          <div className={styles.background43} />
          <div className={styles.equalizer1}>
            <div className={styles.bound6} />
            <div className={styles.rectangle62Copy9} />
            <div className={styles.rectangle62Copy26} />
            <div className={styles.rectangle62Copy46} />
            <div className={styles.rectangle62Copy36} />
          </div>
          <div className={styles.weeklySales}>Weekly Sales</div>
        </div>
      </div>
      <div className={styles.tabs1}>
        <div className={styles.item14}>
          <div className={styles.iconText5}>
            <div className={styles.saasApplication}>
              <p className={styles.saas}>SaaS</p>
              <p className={styles.saas}>Application</p>
            </div>
            <img className={styles.libraryIcon1} alt="" src="/library@2x.png" />
          </div>
        </div>
        <div className={styles.item24}>
          <div className={styles.iconText6}>
            <div className={styles.saasApplication}>
              <p className={styles.saas}>Main</p>
              <p className={styles.saas}>Categories</p>
            </div>
            <img
              className={styles.layout4BlocksIcon4}
              alt=""
              src="/layout4blocks@2x.png"
            />
          </div>
        </div>
        <div className={styles.item34}>
          <div className={styles.iconText7}>
            <div className={styles.saasApplication}>
              <p className={styles.saas}>Video</p>
              <p className={styles.saas}>Tutorials</p>
            </div>
            <img
              className={styles.movieLane2Icon1}
              alt=""
              src="/movielane2@2x.png"
            />
          </div>
        </div>
        <div className={styles.item44}>
          <div className={styles.iconText8}>
            <div className={styles.salesStatistics}>
              <p className={styles.saas}>Sales</p>
              <p className={styles.saas}>Statistics</p>
            </div>
            <div className={styles.equalizer2}>
              <div className={styles.bound6} />
              <div className={styles.rectangle62Copy10} />
              <div className={styles.rectangle62Copy27} />
              <div className={styles.rectangle62Copy47} />
              <div className={styles.rectangle62Copy37} />
            </div>
          </div>
        </div>
        <div className={styles.item53}>
          <div className={styles.iconText9}>
            <div className={styles.arcSecurity}>
              <p className={styles.saas}>ARC</p>
              <p className={styles.saas}>Security</p>
            </div>
            <img
              className={styles.shieldCheckIcon1}
              alt=""
              src="/shieldcheck@2x.png"
            />
          </div>
        </div>
      </div>
      <div className={styles.defaultToggle1}>
        <div className={styles.widgets1}>
          <div className={styles.widget7}>
            <div className={styles.colTitlesBg} />
            <div className={styles.col6}>
              <img className={styles.buttonIcon1} alt="" />
              <div className={styles.buttonBookmark}>
                <div className={styles.background44} />
                <img className={styles.bookmark1Icon1} alt="" />
              </div>
              <div className={styles.button8}>
                <div className={styles.background45} />
                <div className={styles.viewOffer}>View Offer</div>
              </div>
              <div className={styles.button9}>
                <div className={styles.background45} />
                <div className={styles.viewOffer}>View Offer</div>
              </div>
              <div className={styles.button10}>
                <div className={styles.background45} />
                <div className={styles.viewOffer}>View Offer</div>
              </div>
              <div className={styles.button11}>
                <div className={styles.background45} />
                <div className={styles.viewOffer}>View Offer</div>
              </div>
            </div>
            <div className={styles.col51}>
              <div className={styles.item54}>
                <div className={styles.aHero}>A Hero</div>
                <div className={styles.div}>Kobe Leonard</div>
              </div>
              <div className={styles.item45}>
                <div className={styles.avarage}>Avarage</div>
                <img className={styles.starIcon18} alt="" />
                <img className={styles.starIcon19} alt="" />
                <img className={styles.starIcon20} alt="" />
                <img className={styles.halfStartIcon2} alt="" />
                <img className={styles.starIcon21} alt="" />
              </div>
              <div className={styles.item35}>
                <div className={styles.avarage}>Best Rated</div>
                <img className={styles.starIcon18} alt="" />
                <img className={styles.starIcon19} alt="" />
                <img className={styles.starIcon20} alt="" />
                <img className={styles.halfStartIcon2} alt="" />
                <img className={styles.starIcon21} alt="" />
              </div>
              <div className={styles.item25}>
                <div className={styles.aboveAvarage}>Above Avarage</div>
                <img className={styles.starIcon27} alt="" />
                <img className={styles.starIcon28} alt="" />
                <img className={styles.starIcon29} alt="" />
                <img className={styles.starIcon30} alt="" />
                <img className={styles.halfStartIcon3} alt="" />
              </div>
              <div className={styles.item15}>
                <div className={styles.avarage}>Best Rated</div>
                <img className={styles.starIcon18} alt="" />
                <img className={styles.starIcon19} alt="" />
                <img className={styles.starIcon20} alt="" />
                <img className={styles.halfStartIcon2} alt="" />
                <img className={styles.starIcon21} alt="" />
              </div>
              <div className={styles.title}>
                <div className={styles.rating}>RATING</div>
              </div>
            </div>
            <div className={styles.col41}>
              <div className={styles.item54}>
                <div className={styles.aHero}>A Hero</div>
                <div className={styles.div}>Kobe Leonard</div>
              </div>
              <div className={styles.item46}>
                <div className={styles.insurance}>Insurance</div>
                <div className={styles.theHill}>{`The Hill `}</div>
              </div>
              <div className={styles.item36}>
                <div className={styles.insurance}>Transportation</div>
                <div className={styles.theHill}>RoadGee</div>
              </div>
              <div className={styles.item26}>
                <div className={styles.insurance}>{`Houses & Hotels`}</div>
                <div className={styles.theHill}>Agoda</div>
              </div>
              <div className={styles.item16}>
                <div className={styles.insurance}>Web, UI/UX Design</div>
                <div className={styles.theHill}>Intertico</div>
              </div>
              <div className={styles.title1}>
                <div className={styles.rating}>COMPANY</div>
              </div>
            </div>
            <div className={styles.col31}>
              <div className={styles.item54}>
                <div className={styles.paid5}>Paid</div>
                <div className={styles.div}>$130</div>
              </div>
              <div className={styles.item46}>
                <div className={styles.insurance}>Pending</div>
                <div className={styles.theHill}>$14,000</div>
              </div>
              <div className={styles.item36}>
                <div className={styles.insurance}>Paid</div>
                <div className={styles.theHill}>$6,700</div>
              </div>
              <div className={styles.item26}>
                <div className={styles.insurance}>Rejected</div>
                <div className={styles.theHill}>$1,600</div>
              </div>
              <div className={styles.item16}>
                <div className={styles.insurance}>Paid</div>
                <div className={styles.theHill}>$520</div>
              </div>
              <div className={styles.title1}>
                <div className={styles.rating}>COMISSION</div>
              </div>
            </div>
            <div className={styles.col21}>
              <div className={styles.item54}>
                <div className={styles.paid5}>Paid</div>
                <div className={styles.div}>$1,900</div>
              </div>
              <div className={styles.item46}>
                <div className={styles.insurance}>Paid</div>
                <div className={styles.theHill}>$2,600,000</div>
              </div>
              <div className={styles.item36}>
                <div className={styles.insurance}>Paid</div>
                <div className={styles.theHill}>$34,000,000</div>
              </div>
              <div className={styles.item26}>
                <div className={styles.insurance}>Pending</div>
                <div className={styles.theHill}>$23,000,000</div>
              </div>
              <div className={styles.item16}>
                <div className={styles.insurance}>In Proccess</div>
                <div className={styles.theHill}>$8,000,000</div>
              </div>
              <div className={styles.title1}>
                <div className={styles.rating}>EARNINGS</div>
              </div>
            </div>
            <div className={styles.col11}>
              <div className={styles.item58}>
                <div className={styles.htmlAngularjsRuby}>
                  HTML, AngularJS, Ruby
                </div>
                <div className={styles.nataliTrump}>KTR Mobile Application</div>
                <div className={styles.img}>
                  <img className={styles.product71Icon} alt="" />
                </div>
              </div>
              <div className={styles.item49}>
                <div className={styles.htmlAngularjsRuby}>
                  Python, PostgreSQL, ReactJS
                </div>
                <div className={styles.nataliTrump}>Natali Trump</div>
                <img className={styles.avatarIcon3} alt="" />
              </div>
              <div className={styles.item39}>
                <div className={styles.phpLaravelVuejs}>
                  PHP, Laravel, VueJS
                </div>
                <div className={styles.lebronWayde}>Lebron Wayde</div>
                <div className={styles.item5Child} />
                <img className={styles.boy22Icon1} alt="" />
              </div>
              <div className={styles.item29}>
                <div className={styles.cAspnetMs}>C#, ASP.NET, MS SQL</div>
                <div className={styles.jessieClarcson}>Jessie Clarcson</div>
                <img className={styles.avatarIcon3} alt="" />
              </div>
              <div className={styles.item19}>
                <div className={styles.cAspnetMs}>HTML, JS, ReactJS</div>
                <div className={styles.jessieClarcson}>Brad Simmons</div>
                <img className={styles.avatarIcon3} alt="" />
              </div>
              <div className={styles.title4}>
                <div className={styles.products}>PRODUCTS</div>
              </div>
            </div>
            <div className={styles.head3}>
              <div className={styles.button12}>
                <div className={styles.background49} />
                <div className={styles.create}>Create</div>
              </div>
              <div className={styles.button13}>
                <div className={styles.background50} />
                <div className={styles.create}>New Report</div>
              </div>
              <div className={styles.newArrivals}>New Arrivals</div>
              <div className={styles.moreThan4005}>
                More than 400+ new members
              </div>
            </div>
          </div>
          <div className={styles.widget61}>
            <div className={styles.col5}>
              <div className={styles.row5}>
                <img className={styles.buttonTrashIcon10} alt="" />
                <img className={styles.buttonEditIcon10} alt="" />
                <img className={styles.buttonSettingsIcon10} alt="" />
              </div>
              <div className={styles.row4}>
                <img className={styles.buttonTrashIcon10} alt="" />
                <img className={styles.buttonEditIcon10} alt="" />
                <img className={styles.buttonSettingsIcon10} alt="" />
              </div>
              <div className={styles.row3}>
                <img className={styles.buttonTrashIcon10} alt="" />
                <img className={styles.buttonEditIcon10} alt="" />
                <img className={styles.buttonSettingsIcon10} alt="" />
              </div>
              <div className={styles.row2}>
                <img className={styles.buttonTrashIcon10} alt="" />
                <img className={styles.buttonEditIcon10} alt="" />
                <img className={styles.buttonSettingsIcon10} alt="" />
              </div>
              <div className={styles.row1}>
                <img className={styles.buttonTrashIcon10} alt="" />
                <img className={styles.buttonEditIcon10} alt="" />
                <img className={styles.buttonSettingsIcon10} alt="" />
              </div>
            </div>
            <div className={styles.col4}>
              <div className={styles.button2}>
                <div className={styles.background32} />
                <div className={styles.inProgress}>In Progress</div>
              </div>
              <div className={styles.button3}>
                <div className={styles.background33} />
                <div className={styles.rejected}>Rejected</div>
              </div>
              <div className={styles.button4}>
                <div className={styles.background34} />
                <div className={styles.success}>Success</div>
              </div>
              <div className={styles.button5}>
                <div className={styles.background35} />
                <div className={styles.inProgress}>In Progress</div>
              </div>
              <div className={styles.button6}>
                <div className={styles.background36} />
                <div className={styles.approved}>Approved</div>
              </div>
            </div>
            <div className={styles.col3}>
              <div className={styles.reactjsRuby}>{`ReactJS, Ruby `}</div>
              <div className={styles.angularjsC}>AngularJS, C#</div>
              <div className={styles.laravelMetronic}>Laravel, Metronic</div>
              <div className={styles.pythonMysql}>Python, MySQL</div>
              <div className={styles.reactjsHtml}>ReactJs, HTML</div>
            </div>
            <div className={styles.col2}>
              <div className={styles.item5}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$45,200,000</div>
              </div>
              <div className={styles.item4}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$57,000</div>
              </div>
              <div className={styles.item3}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$560,000</div>
              </div>
              <div className={styles.item2}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$4,600,000</div>
              </div>
              <div className={styles.item1}>
                <div className={styles.paid}>Paid</div>
                <div className={styles.div}>$2,000,000</div>
              </div>
            </div>
            <div className={styles.col1}>
              <div className={styles.item51}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.ktrMobileApplication}>
                  KTR Mobile Application
                </div>
                <div className={styles.item5Child} />
                <img className={styles.kickstarterIcon2} alt="" />
              </div>
              <div className={styles.item41}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.hrManagement}>HR Management</div>
                <div className={styles.item5Child} />
                <img className={styles.kickstarterIcon2} alt="" />
              </div>
              <div className={styles.item31}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.ciscoManagement}>Cisco Management</div>
                <div className={styles.item5Child} />
                <img className={styles.kickstarterIcon2} alt="" />
              </div>
              <div className={styles.item21}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.hrManagement}>Telegram Application</div>
                <div className={styles.item5Child} />
                <img className={styles.telegramIcon2} alt="" />
              </div>
              <div className={styles.item11}>
                <div className={styles.ftpBprowbnccc}>
                  <span className={styles.ftp}>FTP:</span>
                  <span className={styles.bprowbnccc}> bprow@bnc.cc</span>
                </div>
                <div className={styles.santOutstanding}>Sant Outstanding</div>
                <div className={styles.item5Child} />
                <img className={styles.telegramIcon2} alt="" />
              </div>
            </div>
            <div className={styles.head4}>
              <div className={styles.tabs}>
                <div className={styles.background37} />
                <div className={styles.day}>Day</div>
                <div className={styles.week}>Week</div>
                <div className={styles.month}>Month</div>
              </div>
              <div className={styles.newArrivals}>New Arrivals</div>
              <div className={styles.moreThan4004}>
                More than 400+ new members
              </div>
            </div>
          </div>
          <div className={styles.widget51}>
            <div className={styles.item52}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" />
            </div>
            <div className={styles.item42}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" />
            </div>
            <div className={styles.item32}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" />
            </div>
            <div className={styles.item22}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" />
            </div>
            <div className={styles.item12}>
              <div className={styles.phpSqliteArtisan}>
                PHP, SQLite, Artisan CLIмм
              </div>
              <div className={styles.brivibaSaas}>Briviba SaaS</div>
              <img className={styles.icon12} alt="" />
            </div>
            <div className={styles.head5}>
              <div className={styles.dots}>
                <div className={styles.dotsChild} />
                <div className={styles.dotsItem} />
              </div>
              <div className={styles.salesProgress}>Sales Progress</div>
            </div>
          </div>
          <div className={styles.widget4}>
            <img className={styles.widget4Child} alt="" />
            <div className={styles.salesChange}>Sales Change</div>
            <div className={styles.div24}>+259</div>
            <img className={styles.icon22} alt="" />
          </div>
          <div className={styles.widget3}>
            <img className={styles.widget3Child} alt="" />
            <div className={styles.salesChange}>Weekly Income</div>
            <div className={styles.div24}>750$</div>
            <img className={styles.icon22} alt="" />
          </div>
          <div className={styles.widget21}>
            <div className={styles.item6}>
              <div className={styles.outlinesKeepYou7}>
                Outlines keep you honest. Indulging in poorly driving and keep
                structure keep you honest and great person
              </div>
              <div className={styles.item6Child} />
              <div className={styles.item6Item} />
              <div className={styles.div5}>23:07</div>
            </div>
            <div className={styles.item512}>
              <div className={styles.newOrderPlacedContainer}>
                <span>{`New order placed  `}</span>
                <span className={styles.usd700}> #XF-2356</span>
              </div>
              <div className={styles.item5Inner} />
              <div className={styles.ellipseDiv} />
              <div className={styles.div27}>21:03</div>
            </div>
            <div className={styles.item43}>
              <div className={styles.outlinesKeepYou5}>
                Outlines keep you honest. Indulging in poorly driving and keep
                structure keep you honest great
              </div>
              <div className={styles.item4Item} />
              <div className={styles.item4Inner} />
              <div className={styles.div5}>16:50</div>
            </div>
            <div className={styles.item33}>
              <div className={styles.makeDepositUsdContainer}>
                <span>{`Make deposit `}</span>
                <span className={styles.usd700}>USD 700</span>
                <span> to ESL</span>
              </div>
              <div className={styles.item3Item} />
              <div className={styles.item3Inner} />
              <div className={styles.div6}>14:37</div>
            </div>
            <div className={styles.item213}>
              <img className={styles.userIcon5} alt="" />
              <img className={styles.userIcon6} alt="" />
              <img className={styles.userIcon7} alt="" />
              <div className={styles.aeolMeetingWith1}>AEOL meeting with</div>
              <div className={styles.item2Child2} />
              <div className={styles.item2Child3} />
              <div className={styles.div30}>10:00</div>
            </div>
            <div className={styles.item13}>
              <div className={styles.outlinesKeepYou5}>
                Outlines keep you honest. Indulging in poorly driving and keep
                structure
              </div>
              <div className={styles.item1Item} />
              <div className={styles.item1Inner} />
              <div className={styles.div5}>08:42</div>
            </div>
            <div className={styles.head6}>
              <div className={styles.sales}>890,344 Sales</div>
              <div className={styles.recentActivities}>Recent Activities</div>
              <div className={styles.dots1}>
                <div className={styles.dotsChild} />
                <div className={styles.dotsItem} />
              </div>
            </div>
          </div>
          <div className={styles.widget11}>
            <div className={styles.background38} />
            <div className={styles.top}>
              <img className={styles.topChild} alt="" />
              <img className={styles.topItem} alt="" />
              <div className={styles.exportButton}>
                <div className={styles.button7} />
                <div className={styles.export}>Export</div>
                <img className={styles.arrowIcon2} alt="" />
              </div>
              <div className={styles.categories}>Categories</div>
            </div>
            <div className={styles.carts1}>
              <div className={styles.cart41}>
                <div className={styles.background58} />
                <div className={styles.bugReports1}>Bug Reports</div>
                <img className={styles.stockholmIconsCommunicatio3} alt="" />
              </div>
              <div className={styles.cart31}>
                <div className={styles.background41} />
                <div className={styles.itemOrders1}>Item Orders</div>
                <img className={styles.compilingIcon5} alt="" />
              </div>
              <div className={styles.cart21}>
                <div className={styles.background42} />
                <div className={styles.newUsers1}>New Users</div>
                <img className={styles.addUserIcon2} alt="" />
              </div>
              <div className={styles.cart11}>
                <div className={styles.background43} />
                <div className={styles.equalizer3}>
                  <div className={styles.bound6} />
                  <div className={styles.rectangle62Copy9} />
                  <div className={styles.rectangle62Copy26} />
                  <div className={styles.rectangle62Copy46} />
                  <div className={styles.rectangle62Copy36} />
                </div>
                <div className={styles.newUsers1}>Weekly Sales</div>
              </div>
            </div>
          </div>
        </div>
        <img className={styles.quickToolsIcon} alt="" />
        <div className={styles.sidebar1}>
          <div className={styles.backgroundRectangle2} />
          <div className={styles.highlightedRectangle4} />
          <div className={styles.topPart1}>
            <div className={styles.highlightedRectangle5} />
            <img className={styles.arrowDoubleLeft1} alt="" />
            <div className={styles.metronic}>METRONIC</div>
          </div>
          <div className={styles.iconsBright}>
            <img className={styles.layersIcon2} alt="" />
            <img className={styles.layout4Blocks2} alt="" />
          </div>
          <div className={styles.headings1}>
            <div className={styles.custom1}>CUSTOM</div>
            <div className={styles.layout}>LAYOUT</div>
            <div className={styles.components}>COMPONENTS</div>
          </div>
          <div className={styles.subheadings1}>
            <div className={styles.users1}>Users</div>
            <div className={styles.contacts1}>Contacts</div>
            <div className={styles.chat1}>Chat</div>
            <div className={styles.pages1}>Pages</div>
            <div className={styles.subheaders}>Subheaders</div>
            <div className={styles.general}>General</div>
            <div className={styles.builder}>Builder</div>
            <div className={styles.base}>Base</div>
            <div className={styles.custom2}>Custom</div>
            <div className={styles.extended}>Extended</div>
            <div className={styles.icons1}>Icons</div>
            <div className={styles.portlets}>Portlets</div>
          </div>
          <div className={styles.subheadingsActive1}>
            <div className={styles.dashboard3}>Dashboard</div>
            <div className={styles.applications1}>Applications</div>
          </div>
          <div className={styles.strokes}>
            <div className={styles.strokesChild} />
            <div className={styles.strokesItem} />
            <div className={styles.strokesInner} />
          </div>
          <img className={styles.iconsFaded1} alt="" />
          <img className={styles.arrowsIcon1} alt="" />
        </div>
        <div className={styles.sunheader1}>
          <div className={styles.background39} />
          <div className={styles.left1}>
            <div className={styles.dashboard4}>Dashboard</div>
            <div className={styles.xrs45670}>#XRS-45670</div>
            <div className={styles.addNewButton3}>
              <div className={styles.addNewButtonChild} />
              <div className={styles.addNew}>Add New</div>
            </div>
            <div className={styles.leftItem} />
          </div>
          <div className={styles.right}>
            <div className={styles.today}>
              <div className={styles.today1}>Today</div>
              <div className={styles.todayActive2}>
                <div className={styles.todayActiveChild} />
                <div className={styles.today1}>Today</div>
              </div>
            </div>
            <div className={styles.month1}>
              <div className={styles.monthActive}>
                <div className={styles.monthActiveChild} />
                <div className={styles.month2}>{`Month `}</div>
              </div>
              <div className={styles.month3}>{`Month `}</div>
            </div>
            <div className={styles.year}>
              <div className={styles.year1}>Year</div>
              <div className={styles.yearActive}>
                <div className={styles.monthActiveChild} />
                <div className={styles.yearActive1}>Year</div>
              </div>
            </div>
            <div className={styles.actionsButtonPassive2}>
              <div className={styles.todayActiveChild} />
              <div className={styles.actions}>Actions</div>
            </div>
            <div className={styles.actionsButtonActive2}>
              <div className={styles.actionsButtonActiveChild} />
              <div className={styles.actions1}>Actions</div>
              <img className={styles.sort1Icon2} alt="" />
            </div>
            <img className={styles.filePlusIcon2} alt="" />
          </div>
        </div>
        <div className={styles.header}>
          <div className={styles.sep2} />
          <div className={styles.right3}>
            <div className={styles.icons2}>
              <img className={styles.searchIcon3} alt="" />
              <img className={styles.compilingIcon6} alt="" />
              <div className={styles.equalizer4}>
                <div className={styles.bound6} />
                <div className={styles.rectangle62Copy12} />
                <div className={styles.rectangle62Copy29} />
                <div className={styles.rectangle62Copy49} />
                <div className={styles.rectangle62Copy39} />
              </div>
              <img className={styles.stockholmIconsShoppingC3} alt="" />
              <img className={styles.layout4BlocksIcon5} alt="" />
              <img className={styles.flagIcon1} alt="" />
            </div>
            <div className={styles.hiSean}>
              <span>Hi,</span>
              <span className={styles.sean}> Sean</span>
            </div>
            <div className={styles.initialIcon1}>
              <div className={styles.rectangle56} />
              <div className={styles.n}>S</div>
            </div>
          </div>
          <div className={styles.left2}>
            <div className={styles.pages2}>
              <div className={styles.todayActiveChild} />
              <div className={styles.pages3}>Pages</div>
            </div>
            <div className={styles.features}>
              <div className={styles.featuresActive}>
                <div className={styles.active2} />
                <div className={styles.featuresActive1}>Features</div>
              </div>
              <div className={styles.features1}>Features</div>
            </div>
            <div className={styles.apps}>
              <div className={styles.appsActive}>
                <div className={styles.active2} />
                <div className={styles.apps1}>Apps</div>
              </div>
              <div className={styles.features1}>Apps</div>
            </div>
          </div>
        </div>
        <img className={styles.toggleIcon2} alt="" />
        <img className={styles.toggleIcon3} alt="" />
      </div>
      <div className={styles.cats}>
        <div className={styles.cart6}>
          <div className={styles.greatGifts}>Great Gifts</div>
          <img
            className={styles.gengen002Icon1}
            alt=""
            src="/gengen002@2x.png"
          />
        </div>
        <div className={styles.cart5}>
          <div className={styles.greatGifts}>Great Gifts</div>
          <img
            className={styles.gengen002Icon1}
            alt=""
            src="/ecmecm002@2x.png"
          />
        </div>
        <div className={styles.cart42}>
          <div className={styles.greatGifts}>Great Gifts</div>
          <img
            className={styles.gengen002Icon1}
            alt=""
            src="/absabs025@2x.png"
          />
        </div>
        <div className={styles.cart32}>
          <div className={styles.greatGifts}>Great Gifts</div>
          <img
            className={styles.gengen002Icon1}
            alt=""
            src="/tehteh004@2x.png"
          />
        </div>
        <div className={styles.cart22}>
          <div className={styles.greatGifts}>Great Gifts</div>
          <img
            className={styles.gengen002Icon1}
            alt=""
            src="/tehteh008@2x.png"
          />
        </div>
        <div className={styles.cart12}>
          <div className={styles.greatGifts}>Great Gifts</div>
          <img
            className={styles.gengen002Icon1}
            alt=""
            src="/ecmecm007@2x.png"
          />
        </div>
      </div>
      <div className={styles.widget22}>
        <div className={styles.chart}>
          <div className={styles.dottedLines}>
            <div className={styles.line14} />
            <div className={styles.line15} />
            <div className={styles.line16} />
            <div className={styles.line17} />
            <div className={styles.line18} />
            <div className={styles.line19} />
          </div>
          <div className={styles.xAxis}>
            <div className={styles.aug}>1 Aug</div>
            <div className={styles.aug1}>8 Aug</div>
            <div className={styles.aug2}>151 Aug</div>
            <div className={styles.aug3}>22 Aug</div>
            <div className={styles.aug4}>29 Aug</div>
            <div className={styles.sep4}>5 Sep</div>
          </div>
          <div className={styles.yAxis}>
            <div className={styles.div32}>150</div>
            <div className={styles.div33}>120</div>
            <div className={styles.div34}>90</div>
            <div className={styles.div35}>60</div>
            <div className={styles.div36}>30</div>
            <div className={styles.div37}>0</div>
          </div>
          <div className={styles.columns}>
            <div className={styles.column}>
              <img
                className={styles.columnChild}
                alt=""
                src="/rectangle-169@2x.png"
              />
              <div className={styles.columnItem} />
            </div>
            <div className={styles.column1}>
              <img
                className={styles.columnInner}
                alt=""
                src="/rectangle-169@2x.png"
              />
              <div className={styles.columnChild1} />
            </div>
            <div className={styles.column2}>
              <img
                className={styles.rectangleIcon}
                alt=""
                src="/rectangle-169@2x.png"
              />
              <div className={styles.columnChild2} />
            </div>
            <div className={styles.column3}>
              <img
                className={styles.columnChild3}
                alt=""
                src="/rectangle-169@2x.png"
              />
              <div className={styles.columnChild4} />
            </div>
            <div className={styles.column4}>
              <img
                className={styles.columnChild5}
                alt=""
                src="/rectangle-169@2x.png"
              />
              <div className={styles.columnChild6} />
            </div>
            <div className={styles.column5}>
              <img
                className={styles.columnChild7}
                alt=""
                src="/rectangle-169@2x.png"
              />
              <div className={styles.columnChild8} />
            </div>
          </div>
        </div>
        <div className={styles.top2}>
          <div className={styles.tabs}>
            <div className={styles.background63} />
            <div className={styles.day}>Day</div>
            <div className={styles.week}>Week</div>
            <div className={styles.month}>Month</div>
          </div>
          <div className={styles.newArrivals}>Recent Stats</div>
          <div className={styles.moreThan4007}>More than 400+ new members</div>
        </div>
      </div>
    </div>
  );
};

export default Option2Advance;
